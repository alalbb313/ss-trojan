...
update_ss(){
	...
	if [ -n "$ss_basic_version_web1" ];then
		...
		echo_date 主服务器在线版本号："$ss_basic_version_web1" 和本地版本号："$ss_basic_version_local" 不同！
			...
			wget --no-check-certificate --timeout=5 "$main_url"/shadowsocks_v$ss_basic_version_web1.tar.gz
			md5sum_gz=`md5sum /tmp/shadowsocks_v$ss_basic_version_web1.tar.gz | sed 's/ /\n/g'| sed -n 1p`
