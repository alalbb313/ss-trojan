...
sync_trojan_binary(){
	trojan_version=`cat ../trojan-go_binary/latest.txt`
	md5_latest=`md5sum ../trojan-go_binary/$trojan_version/trojan-go | sed 's/ /\n/g'| sed -n 1p`
...
do_build() {
	...
	"md5":"$md5value",
	"name":"$MODULE",
	"tar_url": "https://raw.githubusercontent.com/alalbb313/ss-trojan/master/fancyss_arm384/shadowsocks_v$VERSION.tar.gz", 
	"title":"$TITLE",
	"version":"$VERSION"
	}
	EOF
